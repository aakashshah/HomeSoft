	...
	if (isset($_POST['create'])) { //If create button is clicked, check if the fields are not empty
		if ( trim($_POST['usrname']) != "" && trim($_POST['passwd']) != "") {
			...
			if (!mysql_fetch_array($rcSet)) {	
				...
				$query = "INSERT INTO userpass (username, password) VALUES ('$_POST[usrname]', '$pwd')";
				mysql_query($query, $con);
				$query = "CREATE TABLE ".$_POST['usrname']." (expno SMALLINT AUTO_INCREMENT PRIMARY KEY, amount MEDIUMINT NOT NULL, reason VARCHAR(100), dateadded DATE NOT NULL, approved TINYINT NOT NULL)";
				mysql_query($query, $con);
...
<html>
	...
	<body bgcolor="#EEEEEE" onLoad="login.usrname.focus();">
		<br><br>
		<table width="100%" cellpadding="5"><tr><td align="left"><h2>Create User</h2></td></tr></table>
